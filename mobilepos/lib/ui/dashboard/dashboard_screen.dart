...
import '../modals/cash_movement_modal.dart';
...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  Future<void> _checkForUpdate() async {
    // Delay slightly so the UI is fully built before showing dialog
  ...
  Widget build(BuildContext context) {
    // Listen for shift state changes to refresh dashboard data
    ref.listen<PosState>(posProvider, (previous, next) {
      final prevShift = previous?.shift;
      final nextShift = next.shift;
      // Shift was closed (had value, now null) or opened (null, now has value)
      if ((prevShift != null && nextShift == null) ||
          (prevShift == null && nextShift != null)) {
      ...
      children: [
        ...
        Consumer(
          builder: (context, ref, child) {
            ...
            return CashMovementModal(
              isOpen: posState.isCashMovementModalOpen,
              loading: posState.loading,
              onClose: () =>
                  ref.read(posProvider.notifier).toggleCashMovementModal(false),
              onSubmit: (type, amount, note) async {
                try {
                  await ref
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () => ref
                        .read(posProvider.notifier)
                        .toggleCashMovementModal(true),
                    icon: const Icon(LucideIcons.arrowRightLeft, size: 16),
                    label: const Text('Cash Movement'),
                    style: OutlinedButton.styleFrom(
                            ...
                            Text.rich(
                              TextSpan(
                                children: [
                                  ...
                                  TextSpan(
                                    text:
                                        '${alert['current_stock']} ${alert['unit'] is Map ? alert['unit']['abbreviation'] : alert['unit'] ?? '-'}',
                                    style: const TextStyle(
                                      fontWeight: FontWeight.bold,
                                    ),
                                  ),
