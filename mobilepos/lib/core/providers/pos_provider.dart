...
class PosNotifier extends StateNotifier<PosState> {
  ...
  void addToCart(
    ...
    if (existingIndex >= 0) {
      ...
      if (availableMod.name == 'Available' &&
          availableMod.optionName.isNotEmpty) {
  ...
  Future<void> checkout({
    ...
    try {
      ...
      clearCart();
      // Keep customer/type? Vue keeps type, clears customer.
      selectCustomer(null);
  ...
  void loadBillToCart(Map<String, dynamic> bill) {
    ...
    state = state.copyWith(cart: cartItems);
  ...
  Future<void> fetchCurrentShift() async {
    try {
      ...
      state = state.copyWith(shift: null);
    }
  }
